...
i32 main() {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		for (u32 i = 0; i < amount; ++i){
			/*if (sqrt(pow(lightPos.x- models[i].x ,2)+ pow(lightPos.y- models[i].y, 2)+ pow(lightPos.z- models[i].z, 2))<=cubeScale) {
                positions[i].x = 900;
                positions[i].y = 900;
                positions[i].z = 900;
                cubeScale += 0.2f;
...
/* vim: set tabstop=2:softtabstop=2:shiftwidth=2:noexpandtab *//*Hamill Cavero*/
